...
A project in CoSyFeAT consists of a set of System Type Feature Models (STFMS) and one Collaboration Type Feature Model (CTFM). These feature models are represented as a root feature and can be added to the project by right-clicking on the blank space and selecting "Add Featuremodel". Each STFM has two static first-level subfeatures: "Collaborative Context" and "System". These cannot be changed. You can only add subfeatures below. To do so, right-click the respective branch feature and select "Add feature". You can change the name of each system represented in your model by righ-clicking the respective first-level "System" feature and selecting "Change name" - that way you can also change names of any non-static feature that you created specifically for your project. 
